    ...
    <div class="scroll-container">
      ...
      <div class="step-card" :class="{ 'active': phase === 4 }">
        ...
        <div class="card-content">
          ...
          <!-- Retry Button for Prepare -->
          <div v-if="phase >= 3" class="retry-section">
            <button 
              class="retry-btn" 
            ...
            >
              ...
              {{ retryingPrepare ? $t('common.retrying') : $t('common.retryStep') }}
            </button>
          </div>
...
const emit = defineEmits(['go-back', 'next-step', 'add-log', 'update-status', 'retry-prepare'])
...
// Retry prepare
const handleRetryPrepare = async () => {
  if (!props.simulationId) return
  ...
  try {
    const res = await retrySimulationPrepare(props.simulationId)
    if (res.success) {
      // Reset state
      phase.value = 0
      taskId.value = null
